...
class Day17Test extends AnyFunSuite {
  ...
  test("Day17 - Part 1 - exercise") {
    ...
    val trajectory = TargetComputer.trajectoryWithHighestY(targetArea)
    println(trajectory)
    assert(trajectory.initialVelocity === Velocity(24, 75))
    assert(trajectory.steps === 152)
    assert(trajectory.maxProbeHeight === 2850)

  }

  test("Day17 - Part 2 - reference") {
    val input =
      ResourceUtils.getLinesFromResource("day17/reference-input.txt").head
    val targetArea = Day17.parseInput(input)
    val allTrajectories = TargetComputer.allHittingTrajectories(targetArea)
    assert(allTrajectories.length === 112)
  }

  test("Day17 - Part 2 - exercise") {
    val input =
      ResourceUtils.getLinesFromResource("day17/input.txt").head
    val targetArea = Day17.parseInput(input)
    val allTrajectories = TargetComputer.allHittingTrajectories(targetArea)
    assert(allTrajectories.length === 1117)
  }
